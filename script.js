// =================================================================================
// VERSIÓN FINAL - CON SINCRONIZACIÓN EN TIEMPO REAL PARA BOTÓN NATIVO DE WIX
// =================================================================================
...
let chosenColors = { 
    type: 'configUpdate', // Tipo de mensaje para sincronización
    chasis: 'Gris', 
    buttons: {}, 
    knobs: {}    
};
...
// ===== CAMBIO 2: Creamos una función para enviar los datos a Wix =====
function sendConfigToWix() {
    console.log("Enviando actualización de configuración a Wix:", chosenColors);
...
function prepareModelParts() {
    ...
    // ===== CAMBIO 3: Enviamos la configuración inicial a Wix cuando el modelo termina de cargar =====
    sendConfigToWix();
}
    ...
    Object.entries(colors).forEach(([name, colorData]) => {
        ...
        swatch.addEventListener('click', () => {
            if (state.selectedForColoring) {
                ...
                // ===== CAMBIO 5: Enviamos la configuración a Wix CADA VEZ que se cambia un color =====
                sendConfigToWix();
...
// ... (Esta sección no cambia, contiene startIntroAnimation, centerAndScaleModel, etc.) ...
